<script>
    window.BranchUI = window.BranchUI || (function() {
        ...
        function getInitials(value) {
            const words = String(value || 'BR').trim().split(/\s+/).filter(Boolean);
            return words.slice(0, 2).map((word) => word.charAt(0)).join('').toUpperCase() || 'BR';
        }
        ...
        function dataTableOptions(options) {
            return $.extend(true, {
                ...
                dom: '<"branch-table-meta"l>rt<"branch-table-footer"ip>',
                language: {
                    processing: '<span class="branch-loading"><span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>Memuat data...</span>',
        ...
        function initTableTools(config) {
            ...
            $('.branch-refresh-table').on('click', function() {
                const $button = $(this);
        ...
        function initTooltips() {
            ...
            document.querySelectorAll('.branch-page [title]').forEach(function(el) {
        ...
        function clearValidation(formSelector) {
            ...
            $form.find('.branch-field').removeClass('has-error');
        }

        function markInvalid(fieldId, message) {
            $(`#error-${fieldId}`).text(message);
            $(`#${fieldId}`).addClass('is-invalid').closest('.branch-field').addClass('has-error');
        }
