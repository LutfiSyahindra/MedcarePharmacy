...
<head>
    ...
    <title>Surat Pesanan Reguler - {{ $purchaseOrder->no_po }}</title>
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .print-toolbar span { font-size: 12px; }
        ...
        .regular-order-sheet:last-child { page-break-after: auto; break-after: auto; }
        .document-header { margin-top: 2mm; text-align: center; }
        .document-header h1 { margin: 0 0 1mm; font-size: 15pt; text-decoration: underline; }
        .document-number { margin: 0; font-size: 11pt; }
        .section { margin-top: 7mm; }
        .section.compact { margin-top: 4mm; }
        .section > p { margin: 0 0 1.5mm; }
        .field-table { width: 100%; border-collapse: collapse; }
        .field-table td { padding: .65mm 0; vertical-align: top; }
        .field-label { width: 40mm; white-space: nowrap; }
        ...
        .medicine-table .name-column { width: 29%; }
        .medicine-table .unit-column { width: 11%; text-align: center; vertical-align: middle; }
        .medicine-table .quantity-column { width: 9%; text-align: center; vertical-align: middle; }
        .medicine-table .price-column { width: 17%; text-align: right; vertical-align: middle; white-space: nowrap; }
        .medicine-table .total-price-column { width: 19%; text-align: right; vertical-align: middle; white-space: nowrap; }
        .medicine-table .discount-column { width: 15%; text-align: center; vertical-align: middle; }
        .medicine-name { display: block; font-weight: 700; }
        .price-value,
        .total-price-value { font-weight: 700; }
        .discount-tier { display: block; white-space: nowrap; line-height: 1.4; }
        .copy-role-badge {
        ...
        .signature-wrap { display: flex; justify-content: flex-end; margin-top: 7mm; }
        .signature { width: 78mm; text-align: left; }
        .signature p { margin: 0 0 3mm; }
        .signature-space { height: 20mm; }
        .signature-name { display: inline-block; min-width: 62mm; font-weight: 700; text-decoration: underline; }
        .signature-role { margin: .7mm 0; }
        .notes { margin-top: 6mm; font-size: 10pt; }
        ...
        @media print {
            ...
            .regular-order-sheet { width: 190mm; min-height: 277mm; margin: 0; box-shadow: none; }
...
<body>
    @php
        ...
        $copyRoles = [1 => 'Internal Apotek', 2 => 'Distributor'];
    @endphp

    <div class="print-toolbar">
        <span>{{ $regularDetails->count() }} item reguler &middot; 2 lembar: Internal Apotek dan Distributor</span>
    ...
    @for ($copy = 1; $copy <= $copyCount; $copy++)
        @php
            $copyRole = $copyRoles[$copy] ?? 'Salinan';
        @endphp
        <main class="regular-order-sheet">
            <header class="document-header">
                <h1>SURAT PESANAN OBAT REGULER</h1>
                <p class="document-number">Nomor : {{ $purchaseOrder->regular_document_number }}</p>
            ...
            <section class="section compact">
                <p>Mengajukan pesanan obat reguler kepada:</p>
                <table class="field-table">
            ...
            <section class="section compact">
                <p>Dengan rincian obat yang dipesan sebagai berikut:</p>
                    ...
                    <tbody>
                        @foreach ($regularDetails as $detail)
                            @php
                                ...
                                $formatDecimal = static fn ($value) => rtrim(rtrim(number_format((float) $value, 2, ',', '.'), '0'), ',');
                                $quantity = $formatDecimal($detail->qty);
                            ...
                            <tr>
                                ...
                                <td class="unit-column">{{ $unitName }}</td>
                                ...
                                <td class="discount-column">
                                    <span class="discount-tier">D1 {{ $formatDecimal($detail->diskon_1) }}%</span>
                                    <span class="discount-tier">D2 {{ $formatDecimal($detail->diskon_2) }}%</span>
                                    <span class="discount-tier">D3 {{ $formatDecimal($detail->diskon_3) }}%</span>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            <section class="notes">
                <p>Catatan: Surat pesanan ini hanya memuat obat selain Narkotika, Psikotropika, dan Prekursor.</p>
            </section>

            <span class="copy-mark">Lembar {{ $copyRole }} &middot; {{ $copy }} dari {{ $copyCount }}</span>
        </main>
    @endfor
