<script>
    $(document).ready(function() {
        ...
        $('#rakModal').on('show.bs.modal', function() {
            let form = $('#rakForm');
            $('#rakModalLabel').text('ADD RAK PENYIMPANAN OBAT');
            form.trigger('reset');
            $('#submitForm').text('Add');
            $('#addInput').show();
        ...
        // --- DataTable
        let rakTable = $('#tableRak').DataTable({
            processing: true,
            serverSide: true,
            responsive: true,
            autoWidth: false,
        ...
        $('#tableRak').on('change', '.toggle-status', function() {
            let rakId = $(this).data('id');
            let status = $(this).is(':checked') ? 1 : 0;
            console.log('rak ID: ' + rakId + ', Status: ' + status);

            $.ajax({
                url: "{{ route("rak.updateStatus") }}", // pastikan route ini ada
                method: 'PUT',
                data: {
                    _token: '{{ csrf_token() }}',
                    status: status,
                ...
                success: function(response) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Berhasil!',
                        text: 'Status Rak Penyimpnanan berhasil diperbarui.',
                        timer: 1500,
        ...
        $('#searchRak').on('keyup', function() {
            rakTable.search(this.value).draw();
        });
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    if (xhr.status === 422) {
                        ...
                        $('#rakForm').find('.invalid-feedback').text('');
                        $('#rakForm').find('.form-control').removeClass(
                            'is-invalid');
        ...
        window.editRak = function(id) {
            $.ajax({
                ...
                success: function(response) {
                    // isi modal
                    $('#rakModal').modal('show');
                    $('#rakModalLabel').text('EDIT RAK PENYIMPANAN OBAT');
                    ...
                    $('#input-wrapper').html(`
                        <div class="row g-3 mb-2 input-group-item align-items-end">
                            <div class="col-md-3">
                                <label class="form-label">Kode</label>
                                <input type="text" name="kode" value="${response.kode}" class="form-control" placeholder="Kode rak">
                                <div class="invalid-feedback"></div>
                            </div>

                            <div class="col-md-3">
                                <label class="form-label">Rak Penyimpanan Obat</label>
                                <input type="text" name="nama" value="${response.nama}" class="form-control" placeholder="Nama rak">
                                <div class="invalid-feedback"></div>
                            </div>

                            <div class="col-md-4">
                                <label class="form-label">Lokasi</label>
                                <input type="text" name="lokasi" value="${response.lokasi}" class="form-control" placeholder="Lokasi rak">
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                    `);
        ...
        window.deleteRak = function(id) {
            ...
            Swal.fire({
                ...
                text: 'Rak Penyimpanan Obat ini akan dihapus secara permanen!',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route("rak.destroy", ":id") }}".replace(
                            ':id',
                            id),
                        type: 'DELETE',
                        headers: {
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                rakTable.ajax.reload(); // Reload DataTables
                            } else {
                                Swal.fire(
                                    'Gagal!',
                                    response.message,
                        ...
                        error: function(xhr) {
                            Swal.fire(
                                'Gagal!',
                                'Terjadi kesalahan saat menghapus Rak Penyimpanan Obat.',
                                'error'
                            );
                        }
                    });
                    ...
                    if (response.success) {
                        Swal.fire({
                            ...
                            willClose: () => {
                                ...
                                if (typeof rakTable !== 'undefined') {
                                    rakTable.ajax.reload(null,
                                        false
