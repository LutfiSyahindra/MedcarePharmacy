<script>
    window.SatuanUI = window.SatuanUI || (function() {
        ...
        function getInitials(value) {
            const words = String(value || 'ST').trim().split(/\s+/).filter(Boolean);
            return words.slice(0, 2).map((word) => word.charAt(0)).join('').toUpperCase() || 'ST';
        }

        function codeBadge(value) {
            return `
                <span class="satuan-code-badge">
        ...
        function identity(name, subtitle) {
            ...
            const safeSubtitle = escapeHtml(subtitle || 'Satuan obat');

            return `
                <div class="satuan-identity">
                    <span class="satuan-avatar">${escapeHtml(getInitials(name))}</span>
                    <div>
        ...
        function dataTableOptions(options) {
            return $.extend(true, {
                ...
                dom: '<"satuan-table-meta"l>rt<"satuan-table-footer"ip>',
                language: {
                    processing: '<span class="satuan-loading"><span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>Memuat data...</span>',
        ...
        function initTableTools(config) {
            ...
            const $refresh = $(config.refreshSelector || '.satuan-refresh-table');
            let searchTimer = null;

            $search.off('input.satuanTools').on('input.satuanTools', function() {
            ...
            $refresh.off('click.satuanTools').on('click.satuanTools', function() {
                const $button = $(this);
            ...
            $table.find('tbody').off('click.satuanTools').on('click.satuanTools', 'tr', function(event) {
        ...
        function initTooltips() {
            ...
            document.querySelectorAll('.satuan-page [title]').forEach(function(el) {
        ...
        function clearValidation(formSelector) {
            ...
            $form.find('.satuan-field, .satuan-batch-row').removeClass('has-error');
        }
        ...
        function markBatchErrors(formSelector, wrapperSelector, errors) {
            ...
            Object.keys(errors || {}).forEach(function(key) {
                ...
                const $row = index === null ? $(wrapperSelector).find('.satuan-batch-row').first() : $(wrapperSelector).find('.satuan-batch-row').eq(index);
                ...
                $input.closest('.satuan-field').addClass('has-error');
                $row.addClass('has-error');
                $input.closest('.satuan-field').find('.invalid-feedback').first().text(message);
        ...
        function updateBatchCount(wrapperSelector, targetSelector) {
            const count = $(wrapperSelector).find('.satuan-batch-row').length;
