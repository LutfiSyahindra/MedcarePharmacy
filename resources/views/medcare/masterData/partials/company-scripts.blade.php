<script>
    window.CompanyUI = window.CompanyUI || (function() {
        ...
        function getInitials(value) {
            const words = String(value || 'CP').trim().split(/\s+/).filter(Boolean);
            return words.slice(0, 2).map((word) => word.charAt(0)).join('').toUpperCase() || 'CP';
        }

        function codeBadge(value) {
            return `
                <span class="company-code-badge">
        ...
        function contactBadge(value, icon) {
            const text = value || '-';
            return `
                <span class="company-contact-badge" title="${escapeHtml(text)}">
                    <i class="mdi ${icon || 'mdi-information-outline'}"></i>
                    ${escapeHtml(text)}
                </span>
            `;
        }
        ...
        function identity(name, subtitle) {
            ...
            const safeSubtitle = escapeHtml(subtitle || 'Data perusahaan');

            return `
                <div class="company-identity">
                    <span class="company-avatar">${escapeHtml(getInitials(name))}</span>
                    <div>
        ...
        function dataTableOptions(options) {
            return $.extend(true, {
                ...
                dom: '<"company-table-meta"l>rt<"company-table-footer"ip>',
                language: {
                    processing: '<span class="company-loading"><span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>Memuat data...</span>',
        ...
        function initTableTools(config) {
            ...
            const $refresh = $(config.refreshSelector || '.company-refresh-table');
            let searchTimer = null;

            $search.off('input.companyTools').on('input.companyTools', function() {
            ...
            $refresh.off('click.companyTools').on('click.companyTools', function() {
                const $button = $(this);
            ...
            $table.find('tbody').off('click.companyTools').on('click.companyTools', 'tr', function(event) {
        ...
        function initTooltips() {
            ...
            document.querySelectorAll('.company-page [title]').forEach(function(el) {
        ...
        function clearValidation(formSelector) {
            ...
            $form.find('.company-field, .company-batch-row').removeClass('has-error');
        }
        ...
        function markBatchErrors(formSelector, wrapperSelector, errors) {
            ...
            Object.keys(errors || {}).forEach(function(key) {
                ...
                const $row = index === null ? $(wrapperSelector).find('.company-batch-row').first() : $(wrapperSelector).find('.company-batch-row').eq(index);
                ...
                $input.closest('.company-field').addClass('has-error');
                $row.addClass('has-error');
                $input.closest('.company-field').find('.invalid-feedback').first().text(message);
        ...
        function updateBatchCount(wrapperSelector, targetSelector) {
            const count = $(wrapperSelector).find('.company-batch-row').length;
